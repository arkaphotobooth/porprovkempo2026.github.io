...
// --- VARIABEL BROADCAST TV & PENGAMAN REFRESH ---
...
// OTOMATIS MATIKAN TV SAAT LAPTOP DIBUKA/REFRESH (Mencegah TV nyala sendiri)
setTimeout(() => {
    if (DEVICE_COURT && DEVICE_COURT !== 'admin' && DEVICE_COURT !== 'none') {
...
function checkExistingDrawing() {
    ...
    // SEMUA KATEGORI (EMBU & RANDORI) KINI MENGGUNAKAN BAGAN!
...
function filterPesertaScoring(skipTvUpdate = false) {
    ...
    if (stillExists) selectEl.value = currentSelected;
    else if (selectEl.options.length > 0) selectEl.value = selectEl.options[0].value;
...
// =========================================================
// =========================================================
// PENGENDALI MASTER DROPDOWN PESERTA (ANTI-STUCK)
// =========================================================
document.getElementById('select-peserta').addEventListener('change', (e) => { 
    if(e.target.selectedIndex >= 0) { 
        
        // 1. Buka Gembok Timer TV agar TV bisa langsung pindah saat ganti partai
        ...
        if(e.target.value.startsWith('match-')) {
            ...
            if(categoryObj && categoryObj.discipline === 'randori') {
                ...
                // JIKA TV SEDANG LIVE, LANGSUNG UPDATE LAYAR TV KE PARTAI BARU INI!
                if (typeof IS_TV_LIVE !== 'undefined' && IS_TV_LIVE && typeof pushRandoriToTV === 'function') {
                    pushRandoriToTV();
                }
            } else {
                if (typeof currentEmbuMatchId !== 'undefined') currentEmbuMatchId = null;
                loadEmbuMatch(); 
                // Catatan: loadEmbuMatch sudah otomatis mengupdate TV jika sedang Live
            }
        } else { 
            // Jika memilih opsi kosong / "-- Tidak ada Partai --"
            ...
            // Matikan layar TV karena tidak ada partai yang dipilih
            if (typeof IS_TV_LIVE !== 'undefined' && IS_TV_LIVE && typeof DEVICE_COURT !== 'undefined') {
                database.ref(`live_broadcast/${DEVICE_COURT}`).set({ current_action: 'idle' });
            }
        }
...
// =========================================================
function autoGenerateFinal(catName, disciplineType) {
    const poolResults = calculateRandoriFinalists(catName);
    if (!poolResults) return alert("Belum ada hasil pertandingan yang selesai.");
    ...
    if (confirm(`🌟 MAGIC BUTTON AKTIF!\n\nSistem akan otomatis:\n1. Membuat kategori baru bernama "${finalCatName}"\n2. Menyalin Juara 1 & 2 dari Pool A dan B.\n3. Menyiapkan susunan Crossover (Silang).\n\nLanjutkan?`)) {
        ...
        if (!STATE.categories.some(c => c.name === finalCatName)) {
            ...
            STATE.categories.push({
                ...
                name: finalCatName,
                type: originalCat ? originalCat.type : 1,
                discipline: disciplineType // Akan mendeteksi ini Embu atau Randori
            });
        }
...
function renderJuaraUmum() { 
    ...
    STATE.categories.forEach(cat => { 
        ...
        // KARENA EMBU SUDAH PAKAI BAGAN, SEMUA DISIPLIN PAKAI LOGIKA SAMA!
...
function exportMedaliCSV() {
    ...
    STATE.categories.forEach(cat => { 
        ...
        let relatedParticipants = STATE.participants.filter(p => p.kategori.replace(/FINAL/ig, '').trim().toLowerCase() === baseName);
        let uniqueAthletes = new Set(relatedParticipants.map(p => p.nama.toLowerCase().trim()));
        if (uniqueAthletes.size < minPeserta) return; 
...
function exportCustomCSV() { exportRawHasilCSV(null); }
...
function resetAllPenilaian() { 
    if(confirm('⚠️ PERHATIAN: Ini akan MENGHAPUS SEMUA SKOR & PARTAI RANDORI di seluruh jaringan. Yakin?')) { 
        ...
        database.ref().update(updates).then(() => alert('✅ Berhasil: Semua Nilai & Bagan telah di-reset dari server!')).catch(err => alert("Gagal Reset: " + err));
    } 
}
...
function resetDataAtlet() { 
    if(confirm('⚠️ PERHATIAN: Ini MENGHAPUS SEMUA ATLET & BAGAN di seluruh jaringan. Yakin?')) { 
        ...
        updates['turnamen_data/matches'] = null;
        database.ref().update(updates).then(() => alert('✅ Berhasil: Data Atlet dan Bagan telah dihapus dari server!')).catch(err => alert("Gagal Hapus Atlet: " + err));
    } 
}
...
function resetTotalSistem() { 
    if(confirm('🚨 FACTORY RESET: Anda yakin ingin menghapus seluruh sistem (Kategori, Atlet, Nilai) secara permanen dari server?')) { 
        database.ref('turnamen_data').set({ settings: { numJudges: 5, minPesertaJuara: 1 } }).then(() => {
...
function backupDatabase() {
    const dataToBackup = { categories: STATE.categories, participants: STATE.participants, matches: STATE.matches, settings: STATE.settings, backupDate: new Date().toISOString() };
...
function restoreDatabase(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!importedData.categories && !importedData.participants) throw new Error("Format file JSON tidak valid atau bukan file backup MASS.");
            database.ref('turnamen_data').set({
                categories: importedData.categories || [], participants: importedData.participants || [], matches: importedData.matches || [], settings: importedData.settings || { numJudges: 5, minPesertaJuara: 1 }
            }).then(() => alert("✅ RESTORE BERHASIL!\nData turnamen telah dipulihkan dan disinkronkan ke seluruh jaringan.")).catch(err => alert("❌ GAGAL RESTORE ke Server: " + err.message));
...
function saveEmbuCornerScore() {
    ...
    for(let i=1; i<=STATE.settings.numJudges; i++) {
        let sEl = document.getElementById(`score-${i}`);
        if(sEl && sEl.value === "") return alert(`TOTAL NILAI Wasit ${i} belum diisi!`);
    }
...
function finalizeEmbuMatch() {
    ...
    let sMerah = tempEmbuScores.merah;
    let sPutih = tempEmbuScores.putih;

    if(sMerah === 0 && sPutih === 0) return alert("Kedua sudut belum dinilai!");
